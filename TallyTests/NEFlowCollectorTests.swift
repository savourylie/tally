...
final class NEFlowCollectorTests: XCTestCase {
    ...
    /// TICKET-033: when AppState gates the collector off (unhealthy filter), it
    /// must report `.failed` and never poll — so a queued event stays undrained.
    func testMarkUnavailableReportsFailedWithoutPolling() async throws {
        let event = FlowEvent(
            timestamp: 1_779_523_200,
            bundleId: "com.apple.Safari",
            executableName: "Safari",
        ...
        collector.markUnavailable("目前看不到網路使用情況")
        XCTAssertEqual(collector.state, .failed("目前看不到網路使用情況"))

        // The poll loop never started, so the queued event must remain undrained.
        let samples = try await dbPool.read { db in
            try FlowSample.fetchAll(db)
        }
